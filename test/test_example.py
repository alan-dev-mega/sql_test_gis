...
class TestExample(DbTest):
    ...
    def test_count_the_number_of_subordinates(self, conn):
        ...
        sql = """
        SELECT (
            SELECT COUNT(*) FROM enterprise_sales_enterprise_customers a 
            WHERE a.sales_organization_id = b.id
            ) AS subordinates_count , b.id from organizations b ORDER BY b.id ASC
        """
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(sql)
            actual = cur.fetchall()
    ...
    def test_segments_using_geojson_boundary(self, conn):
        ...
        sql = """
        SELECT id FROM japan_segments WHERE 
